...
(function() {
  var RESERVED, clone, coffeescript, contains, fs, installThenWriteBundle, isString, jsp, mb, path, pro, runInExecDir, spawn, wrench, _base, _ref;
  ...
  mb.writeBundle = function(filename, config, options, callback) {
    ...
    dir = path.dirname(filename);
    if (!options.skip_install && (dir !== options.cwd) && path.exists(path.join(dir, 'package.json'))) {
  ...
  mb.writeBundles = function(config, options, callback) {
    ...
    for (filename in config) {
      ...
      count++;
      dir = path.dirname(filename);
      if (!options.skip_install && (dir !== options.cwd) && path.exists(path.join(dir, 'package.json')) && !installed_packages.contains(path)) {
